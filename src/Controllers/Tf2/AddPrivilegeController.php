...
class AddPrivilegeController {
	...
	public function __construct() {
		...
		if ($dbConfig) {
			...
			throw new \Exception('Database configuration for Add Privileges panel not found.');
		}
	}
	...
	public function addPrivileges() {
		...
		include __DIR__ . '/../../Views/tf2/add-privileges.php';
	...
	public function processForm() {
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			$privileges = strip_tags($_POST['privileges']);
			$immunity = strip_tags($_POST['immunity']);
			$days = strip_tags($_POST['days']);
			$discord = isset($_POST['discord']) ? strip_tags($_POST['discord']) : null;
			$comment = isset($_POST['comment']) ? strip_tags($_POST['comment']) : null;

			$error = '';
			$success = '';

			$allowedPrivileges = ['a', 'abcdefgjk'];
			...
			if (!preg_match('/^STEAM_[0-5]:[01]:\d+$/', $steamid)) {
				...
				file_put_contents("tf2/add-privileges.txt", $this->getAccessInfo() . PHP_EOL . print_r($_POST, true) . PHP_EOL, FILE_APPEND);
			}
